...
const PendingBusinessTable = () => {
    ...
    const columns = [
            ...
            render: (_, record) => (
                <Space size="middle" className="align-middle py-1">
                    ...
                    <div className="flex flex-col gap-0.5">
                        <span className="font-bold text-slate-700 text-sm leading-tight">{record.ownerName}</span>
                        <span className="text-[10px] text-slate-400 font-medium tracking-wide">{record.mobileNumber}</span>
                    </div>
                </Space>
            ),
        },
        {
            ...
            className: 'text-xs font-semibold text-slate-700',
            render: (text) => (
                <span className="font-bold text-slate-800 text-xs tracking-tight">{text}</span>
            ),
        },
        {
            title: 'Category',
                    ...
                    <Button 
                        ...
                        onClick={() => {
                            Modal.confirm({
                                ...
                                content: 'Are you sure you want to reject this business listing?',
                                okText: 'Reject',
                                okType: 'danger',
                                cancelText: 'Cancel',
    ...
    return (
        <div className="p-6 bg-white rounded-2xl shadow-sm border border-slate-100 font-sans">
            <div className="mb-6">
                <h2 className="text-xl font-bold text-slate-800 tracking-tight">Pending Verifications</h2>
                <p className="text-slate-400 text-xs mt-1">Review, approve, or reject pending business registration applications</p>
            </div>
