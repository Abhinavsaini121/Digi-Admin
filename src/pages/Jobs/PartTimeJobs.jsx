    ...
    <div className="fixed inset-0 z-[1000] flex items-center justify-center p-4 bg-slate-900/70 backdrop-blur-sm">
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div className="p-4 bg-slate-50 border-t flex justify-end">
          <button
            onClick={onClose}
            className="bg-slate-800 text-white px-6 py-2 rounded-xl text-sm font-bold hover:bg-slate-700 transition-colors"
          >
            Close View
          </button>
        </div>
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
        <div>
          ...
          <p className="text-slate-500 text-sm">
            Create and manage your part-time listings
          </p>
        </div>
        ...
        <div className="flex items-center gap-3">
          <select
            ...
            className="input-field text-blue-300 "
          >
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-50 border-b border-slate-200">
                <th className="p-4 text-xs font-semibold text-slate-500 uppercase w-12">
                  S.No
                </th>
            ...
            <div className="p-6 space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="md:col-span-2">
                  <label className="label-text flex items-center gap-1">
                    <User size={12} /> Select Job Poster (User)*
                  </label>
                  <select
              ...
              <div className="p-5 bg-blue-50/50 rounded-2xl border border-blue-100 space-y-4">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2 text-blue-700 font-extrabold text-[10px] uppercase">
                  ...
                  <button
                    ...
                    disabled={locLoading}
                    className="bg-blue-600 text-white text-[10px] font-bold px-3 py-1.5 rounded-lg flex items-center gap-1"
                  >
                    {locLoading ? (
                ...
                <div className="grid grid-cols-5 gap-3">
                  {createImages.map((file, idx) => (
                    ...
                    >
                      ...
                      <button
                        ...
                        className="absolute top-1 right-1 bg-white text-red-500 rounded-full p-0.5"
                      >
                        <X size={12} />
                      </button>
                    </div>
                  ...
                  {createImages.length < 5 && (
                    <button
                      ...
                      className="aspect-square border-2 border-dashed rounded-xl flex items-center justify-center text-slate-400"
                    >
                      <PlusCircle size={24} />
                    </button>
                  )}
              ...
              <div className="sticky bottom-0 bg-white pt-4 pb-2 flex justify-end gap-3 border-t">
                <button
                  onClick={() => setIsCreateModalOpen(false)}
                  className="px-6 py-2 text-sm font-bold text-slate-400"
                >
                  Cancel
                </button>
                <button
                  ...
                  className="bg-blue-600 text-white px-8 py-2.5 rounded-xl text-sm font-bold flex items-center gap-2"
                >
                  {saveLoading ? (
        ...
        <div className="fixed inset-0 z-[1000] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-md">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-6 space-y-6">
              ...
              <div>
                ...
                <div className="grid grid-cols-5 gap-3 mt-2">
                  ...
                  {selectedJob.images?.map((img, idx) => (
                    <div
                      key={idx}
                      className="relative aspect-square rounded-xl overflow-hidden border border-blue-200"
                    >
                      <img
                        src={img}
                        className="w-full h-full object-cover"
                        alt="Existing"
                      />
                      <div className="absolute top-1 left-1 bg-blue-600 text-white text-[8px] px-1 rounded font-bold">
                        LIVE
                      </div>
                    </div>
                  ))}
                  ...
                  {editNewImages.map((file, idx) => (
                    <div
                      key={idx}
                      className="relative aspect-square rounded-xl overflow-hidden border border-green-200"
                    >
                      <img
                        src={URL.createObjectURL(file)}
                        className="w-full h-full object-cover"
                  ...
                  {(selectedJob.images?.length || 0) + editNewImages.length <
                    ...
                    <button
                      ...
                      className="aspect-square border-2 border-dashed rounded-xl flex items-center justify-center text-slate-400 hover:border-blue-400"
                    >
                      <PlusCircle size={24} />
                    </button>
                  )}
              ...
              <div className="p-5 bg-blue-50/50 rounded-2xl border border-blue-100 space-y-4">
                <div className="flex items-center justify-between">
                  <label className="label-text text-blue-700 font-bold">
                    Job Location
                  </label>
                  <button
                    onClick={() => handleFetchLocation("edit")}
                    disabled={locLoading}
                    className="bg-blue-600 text-white text-[10px] px-2 py-1 rounded-lg flex items-center gap-1"
                  >
                    {locLoading ? (
              ...
              <div className="sticky bottom-0 bg-white pt-4 pb-2 flex justify-end gap-3 border-t">
                ...
                <button
                  ...
                  disabled={saveLoading}
                  className="bg-blue-600 text-white px-8 py-2.5 rounded-xl text-sm font-bold shadow-lg"
                >
                  {saveLoading ? (
                    <Loader2 size={16} className="animate-spin" />
                  ) : (
        ...
        <div className="fixed inset-0 z-[1100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6 text-center">
            ...
            <p className="text-slate-500 text-sm mb-6">
              Do you really want to delete this job? This action cannot be
              undone.
            </p>
...
const ActionBtn = ({ text, variant, onClick, icon }) => {
  ...
  return (
    <button
      onClick={onClick}
      className={`flex items-center gap-1 px-3 py-1.5 text-[10px] font-bold rounded-md border transition-all ${styles[variant]}`}
    >
      {icon} {text}
    </button>
  );
