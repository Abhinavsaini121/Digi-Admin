...
//   const totalPages = Math.ceil(items.length / itemsPerPage); // Make sure this uses items.length
...
//           </div>
//           {/* Line 173 ke baad ye code add karein */}
...
const MarketplaceManager = () => {
  ...
  const fetchMarketplaceData = async (page = currentPage) => {
    ...
    try {
      const token = localStorage.getItem("token");
      const response = await fetch(`https://digiapp-node-1.onrender.com/api/admin/items/Items?page=${page}&limit=${itemsPerPage}`, {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
          "Authorization": `Bearer ${token}`
      ...
      if (result.success) {
        ...
        setTotalPages(result.totalPages || Math.ceil((result.totalItems || 0) / itemsPerPage));
      } else {
        setError("Failed to load data");
      }
    } catch (err) {
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-50/50 border-b border-slate-200 text-slate-400">
                  <th className="p-4 text-xs font-bold uppercase tracking-wider">S.No.</th>
                  <th className="p-4 text-xs font-bold uppercase tracking-wider">Product Info</th>
                  <th className="p-4 text-xs font-bold uppercase tracking-wider">Category</th>
                  <th className="p-4 text-xs font-bold uppercase tracking-wider">Price</th>
                  <th className="p-4 text-xs font-bold uppercase tracking-wider text-center">Featured</th>
          ...
          <div className="flex items-center justify-between px-6 py-4 bg-white border-t border-slate-200">
            <p className="text-sm text-slate-500">
              Showing {indexOfFirstItem + 1} to {Math.min(indexOfFirstItem + items.length, totalItems)} of {totalItems} entries
            </p>
            <div className="flex gap-2">
              <button
                onClick={() => setCurrentPage(prev => Math.max(prev - 1, 1))}
