...
const BannerCard = ({ title, status, date, type, color, onDelete, onEdit }) => (
  <div className="border border-slate-200 rounded-xl overflow-hidden bg-white hover:shadow-lg transition-all group">
    ...
    <div className="p-4">
      ...
      <div className="flex gap-2 mt-4 pt-3 border-t border-slate-50 opacity-0 group-hover:opacity-100 transition-opacity">
        <button
          onClick={onEdit}
          className="flex-1 py-1.5 text-xs font-medium bg-slate-900 text-white rounded"
        >
          Edit
        </button>            <button
          onClick={onDelete}
          className="flex-1 py-1.5 text-xs font-medium bg-red-50 text-red-600 rounded"
        >
          Delete
        </button>
      </div>
