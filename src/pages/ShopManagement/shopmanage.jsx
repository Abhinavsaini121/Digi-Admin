...
const ShopListManagement = () => {
  ...
  // --- NEW: Handle Service Input Change ---
...
const handleAddServiceSubmit = async (e) => {
    ...
    // महत्वपूर्ण सुधार: Key का नाम 'serviceImage' होना चाहिए (जैसा Postman में है)
    if (serviceImages.length > 0) {
        serviceImages.forEach(file => {
            data.append('serviceImage', file); // यहाँ 'serviceImage' ही रखें
        });
    }
    ...
    try {
        ...
        if (response.success) {
            ...
            closeModal();
            fetchInitialData(); // लिस्ट को रिफ्रेश करने के लिए
        }
    } catch (err) {
        // अगर फिर भी 500 एरर आये, तो चेक करें कि बैकएंड single image मांग रहा है या multiple
            ...
            <tbody className="divide-y divide-slate-50">
                ...
                <tr key={shop._id} className="group hover:bg-slate-50/50 transition-all">
                    ...
                    <div className="flex justify-end items-center gap-1">
                        {/* Add Service Button */}
                        <button 
                            onClick={() => handleAction('addService', shop)} 
            ...
            <div className="p-8 overflow-y-auto custom-scrollbar">
                        ...
                        <div className="col-span-2 space-y-4">
                             ...
                             <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div className="space-y-1.5">
                                    ...
                                    <select 
                                        required name="userId" value={formData.userId} onChange={handleInputChange}
                                        className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm font-bold text-slate-700 focus:bg-white focus:ring-4 focus:ring-indigo-500/5 focus:border-indigo-400 outline-none transition-all"
                                    >
                                ...
                                <div className="space-y-1.5">
                                    ...
                                    <input 
                                        ...
                                        className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm font-bold focus:bg-white focus:ring-4 focus:ring-indigo-500/5 focus:border-indigo-400 outline-none transition-all" 
                                    />
                                </div>
                             </div>
                        </div>
                        ...
                        <div className="space-y-4">
                             ...
                             <div className="space-y-4">
                                <div className="space-y-1.5">
                                    ...
                                    <input 
                                        type="text" name="serviceTitle" placeholder="e.g. AC Installation & Maintenance" 
                                        required value={serviceData.serviceTitle} onChange={handleServiceInputChange}
                                        className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm font-bold text-slate-700 focus:bg-white focus:ring-4 focus:ring-indigo-500/5 outline-none transition-all" 
                                    />
                                </div>
                                <div className="space-y-1.5">
                                    <label className="text-[10px] font-bold text-slate-500 uppercase ml-1">Service Details</label>
                                    <textarea 
                                        name="serviceDetails" placeholder="Describe what's included in this service..." 
                                        rows="4" required value={serviceData.serviceDetails} onChange={handleServiceInputChange}
                                        className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm font-bold text-slate-700 focus:bg-white focus:ring-4 focus:ring-indigo-500/5 outline-none transition-all resize-none"
                                    ></textarea>
                                </div>
                             </div>
                        </div>
                             ...
                             <div className="space-y-2">
                                ...
                                <label className="flex flex-col items-center justify-center h-40 bg-slate-50 border-2 border-dashed border-slate-200 rounded-2xl cursor-pointer hover:bg-indigo-50 hover:border-indigo-300 transition-all group">
                                    ...
                                    <span className="text-xs font-bold text-slate-500">
                                        {serviceImages.length > 0 ? `${serviceImages.length} Images Selected` : 'Click to upload service photos'}
                                    </span>
              ...
              {modalType === 'delete' && (
                <div className="text-center py-6">
                   ...
                   <p className="text-slate-500 text-sm px-4">
                      Are you sure you want to remove <span className="font-bold text-slate-900">"{selectedShop?.businessName}"</span>? This action cannot be undone.
                   </p>
                </div>
              )}
              ...
              {modalType === 'approve' && (
                <div className="text-center py-6">
                   ...
                   <p className="text-slate-500 text-sm px-4">
                      By approving, <span className="font-bold text-slate-900">"{selectedShop?.businessName}"</span> will be visible to all users on the platform.
                   </p>
                </div>
              )}
              ...
              {modalType === 'reject' && (
                <div className="text-center py-6">
                   ...
                   <p className="text-slate-500 text-sm px-4">
                      Are you sure you want to reject <span className="font-bold text-slate-900">"{selectedShop?.businessName}"</span>? The owner will need to re-submit details.
                   </p>
                </div>
              )}
                    ...
                    <div className="relative h-60 w-full rounded-3xl overflow-hidden shadow-lg bg-slate-200">
                        ...
                        <div className="absolute bottom-6 left-6 flex items-end gap-5">
                            <div className="w-24 h-24 rounded-2xl border-4 border-white shadow-xl overflow-hidden bg-white">
                                <img src={shopDetail.ownerImage} className="w-full h-full object-cover" alt="Owner" />
                            </div>
                ...
                {modalType === 'view' ? (
                    <>
                        <div className="flex gap-2">
                            {shopDetail?.status !== 'Approved' && (
                                <button onClick={() => { setSelectedShop(shopDetail); setModalType('approve'); }} className="px-6 py-2.5 bg-emerald-600 text-white rounded-xl text-[11px] font-bold uppercase tracking-wider hover:bg-emerald-700 shadow-md transition-all">Approve</button>
                            )}
                            {shopDetail?.status !== 'Rejected' && (
                                <button onClick={() => { setSelectedShop(shopDetail); setModalType('reject'); }} className="px-6 py-2.5 bg-rose-50 text-rose-600 border border-rose-200 rounded-xl text-[11px] font-bold uppercase tracking-wider hover:bg-rose-600 hover:text-white transition-all">Deny / Reject</button>
                            )}
                        </div>
                    ...
                    <>
                        <button type="button" onClick={closeModal} className="px-6 py-2 text-[11px] font-bold text-slate-400 uppercase hover:text-slate-600">Cancel</button>
                        <button form="addShopForm" type="submit" disabled={isProcessing} className="px-10 py-3 bg-indigo-600 text-white rounded-xl text-[11px] font-bold uppercase tracking-widest shadow-lg shadow-indigo-100 hover:bg-indigo-700 disabled:opacity-50 flex items-center gap-2 transition-all">
                            {isProcessing ? <Loader2 className="animate-spin" size={14} /> : <CheckCircle size={14}/>} Save Business
                        </button>
                    </>
                ) : modalType === 'addService' ? (
                    <>
                        <button type="button" onClick={closeModal} className="px-6 py-2 text-[11px] font-bold text-slate-400 uppercase hover:text-slate-600">Cancel</button>
                        <button form="addServiceForm" type="submit" disabled={isProcessing} className="px-10 py-3 bg-indigo-600 text-white rounded-xl text-[11px] font-bold uppercase tracking-widest shadow-lg shadow-indigo-100 hover:bg-indigo-700 disabled:opacity-50 flex items-center gap-2 transition-all">
                            {isProcessing ? <Loader2 className="animate-spin" size={14} /> : <Plus size={14}/>} Add Service
                        </button>
                    </>
                ) : modalType === 'delete' ? (
                    <>
                        <button onClick={closeModal} className="px-6 py-2 text-[11px] font-bold text-slate-400 uppercase">Cancel</button>
                        <button onClick={confirmDelete} disabled={isProcessing} className="px-10 py-3 bg-rose-600 text-white rounded-xl text-[11px] font-bold uppercase tracking-widest shadow-lg shadow-rose-100 hover:bg-rose-700 disabled:opacity-50 flex items-center gap-2 transition-all">
                            {isProcessing ? <Loader2 className="animate-spin" size={14} /> : <Trash2 size={14}/>} Confirm Delete
                        </button>
                    </>
                ) : modalType === 'approve' ? (
                    <>
                        <button onClick={() => setModalType('view')} className="px-6 py-2 text-[11px] font-bold text-slate-400 uppercase">Back</button>
                        <button onClick={handleUpdateStatus} disabled={isProcessing} className="px-10 py-3 bg-emerald-600 text-white rounded-xl text-[11px] font-bold uppercase tracking-widest shadow-lg shadow-emerald-100 hover:bg-emerald-700 disabled:opacity-50 flex items-center gap-2 transition-all">
                            {isProcessing ? <Loader2 className="animate-spin" size={14} /> : <CheckCircle size={14}/>} Confirm Approval
                        </button>
                    </>
                ) : modalType === 'reject' ? (
                    <>
                        <button onClick={() => setModalType('view')} className="px-6 py-2 text-[11px] font-bold text-slate-400 uppercase">Back</button>
                        <button onClick={handleUpdateStatus} disabled={isProcessing} className="px-10 py-3 bg-rose-600 text-white rounded-xl text-[11px] font-bold uppercase tracking-widest shadow-lg shadow-rose-100 hover:bg-rose-700 disabled:opacity-50 flex items-center gap-2 transition-all">
                            {isProcessing ? <Loader2 className="animate-spin" size={14} /> : <XCircle size={14}/>} Confirm Reject
                        </button>
                    </>
                ) : null}
            </div>
