...
import {
  ...
  Edit, Trash2, X, Save, Phone, MessageCircle, Eye
} from "lucide-react";
...
export default function UserMasterProfile() {
  ...
  const mapUserData = (apiData) => {
    return apiData.map(u => ({
      ...
      gender: u.gender || "N/A",
      location: typeof u.location === 'object' ? u.location.address || "Point" : u.location || "N/A",
      ...
      {isViewModalOpen && viewUser && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-2xl overflow-hidden">
            <div className="px-6 py-4 bg-orange-50 border-b border-orange-100 flex justify-between items-center">
              <h2 className="text-lg font-bold text-gray-800 flex items-center gap-2">
                <UserCircle className="text-orange-500" size={22} /> User Detailed Profile
              </h2>
            ...
            <div className="p-6 overflow-y-auto max-h-[75vh]">
              <div className="flex flex-col items-center pb-6 border-b border-gray-100 mb-6">
                ...
                <h3 className="text-xl font-bold text-gray-800 mt-3">{viewUser.name}</h3>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-y-4 gap-x-6 text-sm">
                <div>
                  <span className="block text-xs font-semibold text-gray-400 uppercase tracking-wider">User ID</span>
                  <span className="text-gray-800 font-mono text-xs">{viewUser.id}</span>
                </div>

                <div>
                  <span className="block text-xs font-semibold text-gray-400 uppercase tracking-wider">Verification Status</span>
                  <span className={`inline-flex items-center gap-1 font-medium mt-1 text-xs px-2.5 py-0.5 rounded-full border ${
                    viewUser.isVerified 
                      ? "bg-green-50 text-green-700 border-green-200" 
                ...
                <div>
                  ...
                  <span className="text-gray-800 flex items-center gap-1 mt-1">
                    <Phone size={14} className="text-gray-400" /> {viewUser.mobile}
                  </span>
                </div>

                <div>
                  <span className="block text-xs font-semibold text-gray-400 uppercase tracking-wider">WhatsApp Number</span>
                  <span className="text-gray-800 flex items-center gap-1 mt-1">
                    <MessageCircle size={14} className="text-green-500" /> {viewUser.whatsapp}
                  </span>
                </div>

                <div>
                  <span className="block text-xs font-semibold text-gray-400 uppercase tracking-wider">Gender</span>
                  <span className="text-gray-800 capitalize mt-1 block">{viewUser.gender}</span>
                </div>

                <div>
                  <span className="block text-xs font-semibold text-gray-400 uppercase tracking-wider">Blood Group</span>
                  <span className="text-gray-800 font-bold mt-1 block">{viewUser.blood || "N/A"}</span>
                </div>

                <div className="md:col-span-2">
                  <span className="block text-xs font-semibold text-gray-400 uppercase tracking-wider">Registered Location</span>
                  <span className="text-gray-800 mt-1 block bg-gray-50 p-2.5 rounded-lg border border-gray-100">{viewUser.location}</span>
                </div>

                <div>
                  <span className="block text-xs font-semibold text-gray-400 uppercase tracking-wider">Credits Balance</span>
                  <span className="text-orange-600 font-bold text-lg mt-1 block">{viewUser.credits}</span>
                </div>

                <div>
                  <span className="block text-xs font-semibold text-gray-400 uppercase tracking-wider">Status</span>
                  <span className={`inline-flex items-center font-medium mt-1 text-xs px-2.5 py-0.5 rounded-full border ${
                ...
                <div>
                  <span className="block text-xs font-semibold text-gray-400 uppercase tracking-wider">Joined On</span>
                  <span className="text-gray-600 mt-1 block">{viewUser.joined}</span>
                </div>

                <div>
                  <span className="block text-xs font-semibold text-gray-400 uppercase tracking-wider">Last Activity</span>
                  <span className="text-gray-600 mt-1 block">{viewUser.lastActive}</span>
                </div>
              </div>
            </div>
